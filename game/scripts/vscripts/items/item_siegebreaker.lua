	...
	if IsServer() then
		if self:GetParent():IsRangedAttacker() and params.attacker == self:GetParent() and RollPercentage(self.chance) then
			for _, enemy in ipairs( params.attacker:FindEnemyUnitsInRadius( params.target:GetAbsOrigin(), self.radius ) do
				self:GetAbility:DealDamage(self:GetParent(), params.target, self.damage * params.original_damage, {damage_type = DAMAGE_TYPE_PURE})
			end
		end
	end
end

function modifier_item_siegebreaker:GetModifierAttackRangeBonusUnique()
