...
LinkLuaModifier( "modifier_blade_mail_two", "items/blade_mail/item_blade_mail2.lua" ,LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_blade_mail_two_active", "items/blade_mail/item_blade_mail2.lua" ,LUA_MODIFIER_MOTION_NONE )
...
function item_blade_mail2:GetIntrinsicModifierName()
	return "modifier_blade_mail_two"
end
...
function item_blade_mail2:OnSpellStart()
	...
	if caster:HasModifier("modifier_blade_mail_two_active") then
		caster:RemoveModifierByName("modifier_blade_mail_two_active")
	end
	caster:AddNewModifier(caster,self, "modifier_blade_mail_two_active", {Duration = self:GetSpecialValueFor("duration")})
...
modifier_blade_mail_two = class({})

function modifier_blade_mail_two:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end

function modifier_blade_mail_two:DeclareFunctions(params)
local funcs = {
...
function modifier_blade_mail_two:GetModifierConstantHealthRegen()
	return self:GetAbility():GetSpecialValueFor("health_regen")
end

function modifier_blade_mail_two:GetModifierBonusStats_Strength()
	return self:GetAbility():GetSpecialValueFor("bonus_stats")
end

function modifier_blade_mail_two:GetModifierBonusStats_Intellect()
	return self:GetAbility():GetSpecialValueFor("bonus_stats")
end

function modifier_blade_mail_two:GetModifierBonusStats_Agility()
	return self:GetAbility():GetSpecialValueFor("bonus_stats")
end

function modifier_blade_mail_two:GetModifierPhysicalArmorBonus()
	return self:GetAbility():GetSpecialValueFor("bonus_armor")
end

function modifier_blade_mail_two:GetModifierPreAttack_BonusDamage()
	return self:GetAbility():GetSpecialValueFor("bonus_damage")
end

function modifier_blade_mail_two:IsHidden()
	return true
end

modifier_blade_mail_two_active = class({})
function modifier_blade_mail_two_active:OnCreated( kv )
    self.reflect = self:GetAbility():GetSpecialValueFor( "reflect_pct" )
end

function modifier_blade_mail_two_active:OnRefresh( kv )
    self.reflect = self:GetAbility():GetSpecialValueFor( "reflect_pct" )
end

function modifier_blade_mail_two_active:DeclareFunctions(params)
local funcs = {
...
function modifier_blade_mail_two_active:GetEffectName()
	return "particles/items_fx/blademail.vpcf"
end

function modifier_blade_mail_two_active:GetStatusEffectName()
	return "particles/status_fx/status_effect_blademail.vpcf"
end

function modifier_blade_mail_two_active:StatusEffectPriority()
	return 10
end

function modifier_blade_mail_two_active:IsBuff()
	return true
end

function modifier_blade_mail_two_active:OnTakeDamage(params)
