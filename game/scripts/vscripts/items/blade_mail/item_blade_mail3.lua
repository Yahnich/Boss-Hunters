item_blade_mail3 = class({})
LinkLuaModifier( "modifier_blade_mail_three", "items/blade_mail/item_blade_mail3.lua" ,LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_blade_mail_three_active", "items/blade_mail/item_blade_mail3.lua" ,LUA_MODIFIER_MOTION_NONE )

function item_blade_mail3:PiercesDisableResistance()
    return true
end

function item_blade_mail3:GetIntrinsicModifierName()
	return "modifier_blade_mail_three"
end

function item_blade_mail3:OnSpellStart()
	local caster = self:GetCaster()

	EmitSoundOn("Hero_Axe.Berserkers_Call", caster)
	EmitSoundOn("DOTA_Item.BladeMail.Activate", caster)

	if caster:HasModifier("modifier_blade_mail_three_active") then
		caster:RemoveModifierByName("modifier_blade_mail_three_active")
	end
	caster:AddNewModifier(caster,self, "modifier_blade_mail_three_active", {Duration = self:GetSpecialValueFor("duration")})
...
modifier_blade_mail_three = class({})

function modifier_blade_mail_three:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end

function modifier_blade_mail_three:DeclareFunctions(params)
local funcs = {
...
function modifier_blade_mail_three:GetModifierConstantHealthRegen()
	return self:GetAbility():GetSpecialValueFor("health_regen")
end

function modifier_blade_mail_three:GetModifierBonusStats_Strength()
	return self:GetAbility():GetSpecialValueFor("bonus_stats")
end

function modifier_blade_mail_three:GetModifierBonusStats_Intellect()
	return self:GetAbility():GetSpecialValueFor("bonus_stats")
end

function modifier_blade_mail_three:GetModifierBonusStats_Agility()
	return self:GetAbility():GetSpecialValueFor("bonus_stats")
end

function modifier_blade_mail_three:GetModifierPhysicalArmorBonus()
	return self:GetAbility():GetSpecialValueFor("bonus_armor")
end

function modifier_blade_mail_three:GetModifierPreAttack_BonusDamage()
	return self:GetAbility():GetSpecialValueFor("bonus_damage")
end

function modifier_blade_mail_three:IsHidden()
	return true
end

modifier_blade_mail_three_active = class({})
function modifier_blade_mail_three_active:OnCreated( kv )
    self.reflect = self:GetAbility():GetSpecialValueFor( "reflect_pct" )
end

function modifier_blade_mail_three_active:OnRefresh( kv )
    self.reflect = self:GetAbility():GetSpecialValueFor( "reflect_pct" )
end

function modifier_blade_mail_three_active:DeclareFunctions(params)
local funcs = {
...
function modifier_blade_mail_three_active:GetEffectName()
	return "particles/items_fx/blademail.vpcf"
end

function modifier_blade_mail_three_active:GetStatusEffectName()
	return "particles/status_fx/status_effect_blademail.vpcf"
end

function modifier_blade_mail_three_active:StatusEffectPriority()
	return 10
end

function modifier_blade_mail_three_active:IsBuff()
	return true
end

function modifier_blade_mail_three_active:OnTakeDamage(params)
