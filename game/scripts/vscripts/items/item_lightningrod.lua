item_lightningrod = class({})
LinkLuaModifier( "modifier_item_lightningrod_handle", "items/item_lightningrod.lua", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_item_lightningrod_handle_damage", "items/item_lightningrod.lua", LUA_MODIFIER_MOTION_NONE )

function item_lightningrod:GetIntrinsicModifierName()
	return "modifier_item_lightningrod_handle"
end

function item_lightningrod:OnSpellStart()
...
function modifier_item_lightningrod_handle:OnAttackLanded(params)
	if IsServer() then
		if params.attacker == self:GetParent() and params.target:IsAlive() and RollPercentage(self:GetSpecialValueFor("strike_chance")) then
			...
			target:AddNewModifier(caster, ability, "modifier_item_lightningrod_handle_damage", {})
		end
	end
end

function modifier_item_lightningrod_handle:IsHidden()
	return true
end
	    ...
	    function()
			...
			if ability.strike_bounces[current] > 0 then
				...
				if new_target ~= nil then
					...
					new_target:AddNewModifier(caster, ability, "modifier_item_lightningrod_handle_damage", {})
...
function modifier_item_lightningrod_handle_damage:IsHidden()
	return true
end