relic_longbow = class(relicBaseClass)

function relic_longbow:OnCreated()
	if IsServer() then
...
function relic_longbow:OnDestroy()
	if IsServer() then
...
function relic_longbow:DeclareFunctions()
	return {MODIFIER_PROPERTY_ATTACK_RANGE_BONUS, MODIFIER_PROPERTY_PROJECTILE_SPEED_BONUS}
end

function relic_longbow:GetModifierAttackRangeBonus()
	return 200
end

function relic_longbow:GetModifierProjectileSpeedBonus()
	return 900
end