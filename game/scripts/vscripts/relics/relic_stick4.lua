relic_stick4 = class(relicBaseClass)

function relic_stick4:OnCreated()
...
function relic_stick4:OnEventFinished(args)
	EVENT_TYPE_EVENT = 3
	if args.eventType ~= EVENT_TYPE_EVENT then
		...
		if self:GetStackCount() >= 600 then
			...
			parent.ownedRelics[ability:entindex()] = "relic_stick5"
			LinkLuaModifier("relic_stick5", "relics/generic/relic_stick5", LUA_MODIFIER_MOTION_NONE)
			parent:AddNewModifier( parent, ability, "relic_stick5", {})
...
function relic_stick4:OnDestroy()
	if IsServer() then
		EventManager:UnsubscribeListener("boss_hunters_event_finished", self.funcID)
	end
end

function relic_stick4:DeclareFunctions()
	return {MODIFIER_PROPERTY_PREATTACK_BONUS_DAMAGE}
end

function relic_stick4:GetModifierPreAttack_BonusDamage()
