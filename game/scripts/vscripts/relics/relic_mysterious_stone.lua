relic_mysterious_stone = class(relicBaseClass)

function relic_mysterious_stone:OnIntervalThink()
	self:SetDuration(-1, true)
	self:StartIntervalThink(-1)
end

function relic_mysterious_stone:DeclareFunctions()
	return {MODIFIER_PROPERTY_STATUS_RESISTANCE_STACKING, MODIFIER_PROPERTY_ABSORB_SPELL  }
end

function relic_mysterious_stone:GetModifierStatusResistanceStacking()
	if not self:GetParent():HasModifier("relic_ritual_candle") then return -50 end
end

function relic_mysterious_stone:GetAbsorbSpell(params)
