...
function relic_the_pact:GetModifierMagicalResistanceBonus()
	if not self:GetParent():HasModifier("relic_ritual_candle") then return -33 end
end

function relic_the_pact:GetModifierPhysicalArmorBonus()
...
function relic_the_pact:GetModifierHealAmplify_Percentage(params)
	if not self:GetParent():HasModifier("relic_ritual_candle") and params.ability ~= self:GetAbility() then 
		return -999 
	end
end