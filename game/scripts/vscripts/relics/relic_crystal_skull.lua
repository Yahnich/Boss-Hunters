...
function relic_crystal_skull:GetModifierPhysicalArmorBonus()
	if not self:GetParent():HasModifier("relic_ritual_candle") then return (self.total_armor or 0) * (-1) end
end

function relic_crystal_skull:GetModifierMagicalResistanceBonus()
	if not self:GetParent():HasModifier("relic_ritual_candle") then return (self.magic_resistance or 0) * (-1) end
end
