...
function relic_unchanging_globe:OnCreated()
...
function relic_unchanging_globe:GetModifierManaBonus()
	if not self:GetParent():HasModifier("relic_ritual_candle") then return self.mana end
end

function relic_unchanging_globe:GetModifierConstantManaRegen(params)
	if not self:GetParent():HasModifier("relic_ritual_candle") then return - self:GetStackCount() / 100 end
end
...
function relic_unchanging_globe:OnModifierAdded(params)
