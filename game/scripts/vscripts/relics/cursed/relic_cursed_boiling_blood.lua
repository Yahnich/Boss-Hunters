...
function relic_cursed_boiling_blood:OnIntervalThink()
	...
	if #enemies > 0 and not self:GetParent():HasModifier("relic_unique_ritual_candle") then ApplyDamage({victim = parent, attacker = parent, damage = math.floor(parent:GetHealth() * 0.02), damage_type = DAMAGE_TYPE_MAGICAL, damage_flags = DOTA_DAMAGE_FLAG_NON_LETHAL + DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION }) end
end