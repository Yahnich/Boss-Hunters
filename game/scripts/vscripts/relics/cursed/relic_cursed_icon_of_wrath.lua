...
function relic_cursed_icon_of_wrath:GetModifierIncomingDamage_Percentage(params)
	ApplyDamage({victim = params.attacker, attacker = self:GetParent(), damage = self:GetParent():GetPrimaryStatValue(), damage_type = DAMAGE_TYPE_MAGICAL, damage_flags = DOTA_DAMAGE_FLAG_REFLECTION})
	return 25
end
