relic_generic_stick5 = class(relicBaseClass)

function relic_generic_stick5:OnCreated()
...
function relic_generic_stick5:OnEventFinished(args)
	EVENT_TYPE_EVENT = 3
...
function relic_generic_stick5:OnDestroy()
	if IsServer() then
		EventManager:UnsubscribeListener("boss_hunters_event_finished", self.funcID)
	end
end

function relic_generic_stick5:DeclareFunctions()
	return {MODIFIER_PROPERTY_PREATTACK_BONUS_DAMAGE}
end

function relic_generic_stick5:GetModifierPreAttack_BonusDamage()
