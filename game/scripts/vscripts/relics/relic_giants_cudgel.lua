...
function relic_giants_cudgel:OnIntervalThink()
	self.as = 0
	self.as = self:GetParent():GetIncreasedAttackSpeed() * (-40)
end
...
function relic_giants_cudgel:GetModifierAttackSpeedBonus()
	if not self:GetParent():HasModifier("relic_ritual_candle") then return self.as end
end
