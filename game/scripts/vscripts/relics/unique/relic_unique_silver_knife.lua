relic_unique_silver_knife = class(relicBaseClass)

if IsServer() then
	function relic_unique_silver_knife:OnCreated()
		self:StartIntervalThink(0.33)
	end
	
	function relic_unique_silver_knife:OnIntervalThink()
		stacks = 0
...
function relic_unique_silver_knife:DeclareFunctions()
	return {MODIFIER_PROPERTY_PREATTACK_BONUS_DAMAGE }
end

function relic_unique_silver_knife:GetModifierPreAttack_BonusDamage()
	return 15 * self:GetStackCount()
end

function relic_unique_silver_knife:IsHidden()
	return false
end