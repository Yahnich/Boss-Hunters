relic_charons_obol = class(relicBaseClass)

function relic_charons_obol:DeclareFunctions()
	return {MODIFIER_PROPERTY_INCOMING_DAMAGE_PERCENTAGE, MODIFIER_PROPERTY_MIN_HEALTH}
end

function relic_charons_obol:OnIntervalThink()
	self:StartIntervalThink(-1)
	self:SetDuration(-1, true)
end

function relic_charons_obol:GetModifierIncomingDamage_Percentage(params)
...
function relic_charons_obol:GetMinHealth()
	if self:GetDuration() == -1 then
		return 1
	end
end

function relic_charons_obol:IsHidden()
