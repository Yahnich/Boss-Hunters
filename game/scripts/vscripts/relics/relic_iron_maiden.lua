relic_iron_maiden = class(relicBaseClass)

function relic_iron_maiden:DeclareFunctions()
	return {MODIFIER_EVENT_ON_TAKEDAMAGE, MODIFIER_PROPERTY_INCOMING_DAMAGE_PERCENTAGE}
end

function relic_iron_maiden:GetModifierIncomingDamage_Percentage()
	if not self:GetParent():HasModifier("relic_ritual_candle") then return 50 end
end

function relic_iron_maiden:OnTakeDamage(params)
