...
function AbilityManager:LoadTalentData()
	self.talentKV = {}
...
function AbilityManager:ApplyTalentModifier( hero, talentType )
...
function AbilityManager:GetTalentDataForType( talentType, talentTier )
	if self.talentKV == nil then
		self:LoadTalentData()
	end
...
function AbilityManager:ParseInformationRequest(userid, request)
...
function AbilityManager:IsPlayerRegistered(hero)
	return hero.statsHaveBeenRegistered or false
end

function AbilityManager:RegisterPlayer(hero, bRespec)
	...
	for i = 0, hero:GetAbilityCount() - 1 do
        ...
        if ability then
			...
			if abName ~= nil and abName ~= '' then
				talents.uniqueTalents[abName] = AbilityManager:GetAbilityLinkedTalents(abName, hero)
			end
        end
    end
...
function AbilityManager:GetAbilityLinkedTalents( abilityName, hero )
	local talents = {}
...
function AbilityManager:ProcessUniqueTalents(userid, event)
	local pID = event.pID
...
function AbilityManager:ProcessHeroMasteries(userid, event)
	local pID = event.pID
...
function AbilityManager:NotifyTalent(userid, event)
...
function AbilityManager:RespecAll(userid, event)
	local pID = event.pID
	local entindex = event.entindex
	local hero = EntIndexToHScript( entindex )
