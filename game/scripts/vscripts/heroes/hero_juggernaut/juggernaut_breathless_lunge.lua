...
function modifier_juggernaut_breathless_lunge_dash:OnCreated()
	...
	self.bleed_duration = self:GetSpecialValueFor("bleed_duration")
...
function modifier_juggernaut_breathless_lunge_dash:DeclareFunctions()
	return {MODIFIER_PROPERTY_PREATTACK_BONUS_DAMAGE}
end

function modifier_juggernaut_breathless_lunge_dash:GetModifierPreAttack_BonusDamage()
	...
	if parent:IsAlive() and self.distanceTraveled < self.distance then
		...
		for _, enemies in ipairs(caster:FindEnemyUnitsInLine(oldPos, newPos, parent:GetAttackRange())) do
			if not self.enemies_hit[enemies:entindex()] then
				...
				if self.bleed_duration > 0 then
					enemies:AddNewModifier(caster, self:GetAbility(), "modifier_juggernaut_breathless_lunge_bleed", {duration =  self.bleed_duration})
				end

				if self.disarm_duration > 0 then
					enemies:Disarm(self:GetAbility(), caster, self.disarm_duration)
				end
