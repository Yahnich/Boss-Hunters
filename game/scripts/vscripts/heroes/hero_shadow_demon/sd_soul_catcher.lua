...
function modifier_sd_soul_catcher:OnDeath(params)
	if IsServer() then
		local caster = self:GetCaster()
		if caster:HasTalent("special_bonus_unique_sd_soul_catcher_2") and params.unit == self:GetParent() then
			caster:ConjureImage( {outgoing_damage = 0, incoming_damage = 200, position = params.unit:GetAbsOrigin(), controllable = false}, 5, caster, illusions )
		end
	end
end
