			...
			if params.inflictor and params.inflictor ~= self:GetParent():FindAbilityByName("sd_void_release") and not params.inflictor:IsItem() then
				if RollPercentage(self:GetSpecialValueFor("chance")) then
					if params.unit:HasModifier("modifier_sd_shadow_poison") then
						self:GetAbility():FireTrackingProjectile("particles/units/heros/hero_shadow_demon/shadow_demon_projection.vpcf", params.unit, 500, {}, DOTA_PROJECTILE_ATTACHMENT_HITLOCATION, true, true, 500)
					end
				end
			end
		end
