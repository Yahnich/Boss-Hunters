...
function lion_mana_aura:GetCooldown(lvl)
	return TernaryOperator( self:GetSpecialValueFor("scepter_cd"), self:GetCaster():HasScepter(), 0 )
end
...
function modifier_lion_mana_aura:OnCreated()
    ...
    self.damage = self:GetSpecialValueFor("curr_mana_death") / 100
    self.radius = self:GetSpecialValueFor("death_radius")
end
