...
function undying_necropolis:OnSpellStart()
	...
	if target then
		local hideaway = target:AddNewModifier( caster, self, "modifier_undying_necropolis_undying", {duration = duration} )
...
function undying_necropolis:SummonZombie( unit, duration )
	...
	local zombie = caster:CreateSummon("npc_dota_unit_undying_tombstone_zombie", (unit or caster):GetAbsOrigin(), duration, false)
	...
	if IsEntitySafe( unit ) then
		kv.unit = unit:entindex()
	end
	zombie:AddNewModifier(caster, self, "modifier_undying_necropolis_zombie", kv)
...
function modifier_undying_necropolis_undying:OnIntervalThink()
	...
	if CalculateDistance( parent, self.tombstone ) > 128 then
		self.tombstone._hideawayUnit = nil
		self:Destroy()
		return
	end
	self.tombstone:ModifyThreat( parent:GetThreat(), true )
	parent:SetThreat( 0 )
end
...
function modifier_undying_necropolis_undying:DeclareFunctions()
	return {MODIFIER_PROPERTY_INVISIBILITY_LEVEL}
end

function modifier_undying_necropolis_undying:GetModifierInvisibilityLevel()
	return 1.0
end
...
function modifier_undying_necropolis_tombstone:OnOrder( params )
	...
	if params.unit._lastOrderedTombstoneMove then
		params.unit._lastOrderedTombstoneMove = nil
	end
	if not params.target then return end
	if params.order_type ==  DOTA_UNIT_ORDER_MOVE_TO_TARGET
	or params.order_type ==  DOTA_UNIT_ORDER_ATTACK_TARGET then
		local orderUnit = params.unit
		local parent = self:GetParent()
		if params.target ~= parent then return end
		if orderUnit._lastOrderedTombstoneMove then return end -- don't spam tombstone orders if its already happening
		orderUnit._lastOrderedTombstoneMove = true
		Timers:CreateTimer( 0.1, function()
			if not ( IsEntitySafe( parent ) and IsEntitySafe( orderUnit ) ) then return end
			if not orderUnit._lastOrderedTombstoneMove then return end
			if CalculateDistance( parent, orderUnit ) <= 128 then
				if self.enter_tombstone and not parent._hideawayUnit then
					local hideaway = params.unit:AddNewModifier( caster, self:GetAbility(), "modifier_undying_necropolis_undying", {duration = self:GetRemainingTime()} )
					hideaway.tombstone = parent
					parent._hideawayUnit = params.unit
					params.unit:SetAbsOrigin( parent:GetAbsOrigin() )
				end
				if params.unit == caster and self.pickup_tombstone then
					parent:AddNewModifier( caster, self:GetAbility(), "modifier_undying_necropolis_legion", {duration = self:GetRemainingTime()} )
					parent:SetParent( caster, "attach_hitloc" )
					parent:SetLocalOrigin( Vector(0, 0, 0) )
				end
			else
				return 0.1
			end
...
function modifier_undying_necropolis_tombstone:GetModifierHealthBarPips()
	return self:GetParent():GetMaxHealth() / self.dmgBoss
end
...
function modifier_undying_necropolis_zombie:GetModifierHealthBarPips()
	return self:GetParent():GetMaxHealth() / self.dmgBoss
end
