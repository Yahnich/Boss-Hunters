...
function centaur_hoof_stomp_ebf:GetCastRange(target, position)
	return ( self:GetSpecialValueFor("radius") + self:GetCaster():GetStrength() * self:GetSpecialValueFor("str_to_radius") / 100 )
end

function centaur_hoof_stomp_ebf:OnSpellStart()
	local caster = self:GetCaster()
	
	local radius =  self:GetSpecialValueFor("radius") + self:GetCaster():GetStrength() * self:GetSpecialValueFor("str_to_radius") / 100
	local duration = self:GetSpecialValueFor("stun_duration")
	local damage = self:GetSpecialValueFor("stomp_damage") + caster:GetStrength() * self:GetSpecialValueFor("str_to_damage") / 100
	...
	if caster:HasTalent("special_bonus_unique_centaur_hoof_stomp_1") then
		...
		if stampede then
			caster:AddNewModifier(caster, stampede, "modifier_centaur_stampede_ebf", {duration = stampede:GetSpecialValueFor("duration")})
		end
	end
