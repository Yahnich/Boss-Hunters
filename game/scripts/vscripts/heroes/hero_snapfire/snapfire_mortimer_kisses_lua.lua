...
LinkLuaModifier("modifier_snapfire_mortimer_kisses_lua_buff", "heroes/hero_snapfire/snapfire_mortimer_kisses_lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_snapfire_mortimer_kisses_lua_targeting", "heroes/hero_snapfire/snapfire_mortimer_kisses_lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_snapfire_mortimer_kisses_lua_fire", "heroes/hero_snapfire/snapfire_mortimer_kisses_lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_snapfire_mortimer_kisses_lua_fire_damage", "heroes/hero_snapfire/snapfire_mortimer_kisses_lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_snapfire_mortimer_kisses_lua_talent", "heroes/hero_snapfire/snapfire_mortimer_kisses_lua", LUA_MODIFIER_MOTION_NONE)

function snapfire_mortimer_kisses_lua:IsStealable()
	return true
end

function snapfire_mortimer_kisses_lua:IsHiddenWhenStolen()
	return false
end
...
function snapfire_mortimer_kisses_lua:OnSpellStart()
	...
	caster:AddNewModifier(caster, self, "modifier_snapfire_mortimer_kisses_lua_buff", {Duration = duration})

	if caster:HasTalent("special_bonus_unique_snapfire_mortimer_kisses_lua_2") then
		caster:AddNewModifier(caster, self, "modifier_snapfire_mortimer_kisses_lua_talent", {Duration = duration})
	end
end

function snapfire_mortimer_kisses_lua:OnProjectileHit(hTarget, vLocation)
	if hTarget then
...
function snapfire_mortimer_kisses_lua:CreateFirePit(vLocation)
	local caster = self:GetCaster()
	...
	CreateModifierThinker(caster, self, "modifier_snapfire_mortimer_kisses_lua_fire", {Duration = duration}, vLocation, caster:GetTeam(), false)
end

modifier_snapfire_mortimer_kisses_lua_buff = class({})

function modifier_snapfire_mortimer_kisses_lua_buff:OnCreated(table)
    if IsServer() then 
		...
		if parent:HasTalent("special_bonus_unique_snapfire_mortimer_kisses_lua_1") then
...
function modifier_snapfire_mortimer_kisses_lua_buff:OnRefresh(table)
    if IsServer() then 
        local parent = self:GetParent()
...
function modifier_snapfire_mortimer_kisses_lua_buff:OnIntervalThink()
	if IsServer() then
		local parent = self:GetParent()
...
function modifier_snapfire_mortimer_kisses_lua_buff:OnRemoved()
	if IsServer() then
		local parent = self:GetParent()
...
function modifier_snapfire_mortimer_kisses_lua_buff:CheckState()
	return { [MODIFIER_STATE_ROOTED] = true,
			 [MODIFIER_STATE_DISARMED] = true }
end

function modifier_snapfire_mortimer_kisses_lua_buff:DeclareFunctions()
...
function modifier_snapfire_mortimer_kisses_lua_buff:GetActivityTranslationModifiers()
	return self.animationTranslator
end

function modifier_snapfire_mortimer_kisses_lua_buff:OnAbilityExecuted(params)
	if IsServer() then
		local parent = self:GetParent()
...
function modifier_snapfire_mortimer_kisses_lua_buff:OnOrder(params)
    if IsServer() then 
        local parent = self:GetParent()
        local unit = params.unit
...
function modifier_snapfire_mortimer_kisses_lua_buff:OnAttackRecord(params)
	if IsServer() then
		local parent = self:GetParent()
...
function modifier_snapfire_mortimer_kisses_lua_buff:GetModifierTurnRate_Override()
    return self.turn_rate
end

function modifier_snapfire_mortimer_kisses_lua_buff:GetOverrideAnimation()
    return ACT_DOTA_OVERRIDE_ABILITY_4
end

function modifier_snapfire_mortimer_kisses_lua_buff:IsPurgable()
    return true
end

function modifier_snapfire_mortimer_kisses_lua_buff:IsDebuff()
    return false
end

modifier_snapfire_mortimer_kisses_lua_targeting = class({})

function modifier_snapfire_mortimer_kisses_lua_targeting:OnCreated(table)
    if IsServer() then 
        local parent = self:GetParent()
        local caster = self:GetCaster()
...
modifier_snapfire_mortimer_kisses_lua_fire = class({})

function modifier_snapfire_mortimer_kisses_lua_fire:OnCreated(table)
    if IsServer() then 
        local parent = self:GetParent()
        local caster = self:GetCaster()
...
function modifier_snapfire_mortimer_kisses_lua_fire:OnRemoved()
	if IsServer() then
		StopSoundOn("Hero_Snapfire.MortimerBlob.Magma", self:GetParent())
	end
end

function modifier_snapfire_mortimer_kisses_lua_fire:IsAura()
	return true
end

function modifier_snapfire_mortimer_kisses_lua_fire:GetAuraRadius()
	return self.radius
end

function modifier_snapfire_mortimer_kisses_lua_fire:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_ENEMY
end

function modifier_snapfire_mortimer_kisses_lua_fire:GetAuraSearchType()
	return DOTA_UNIT_TARGET_ALL
end

function modifier_snapfire_mortimer_kisses_lua_fire:GetAuraSearchFlags()
	return DOTA_UNIT_TARGET_FLAG_NONE
end

function modifier_snapfire_mortimer_kisses_lua_fire:GetAuraDuration()
	return self.burn_linger_duration
end

function modifier_snapfire_mortimer_kisses_lua_fire:GetModifierAura()
	return "modifier_snapfire_mortimer_kisses_lua_fire_damage"
end

modifier_snapfire_mortimer_kisses_lua_fire_damage = class({})

function modifier_snapfire_mortimer_kisses_lua_fire_damage:OnCreated(table)
...
function modifier_snapfire_mortimer_kisses_lua_fire_damage:OnRefresh(table)
    if IsServer() then 
        self:StartIntervalThink(self.burn_rate)
    end
end

function modifier_snapfire_mortimer_kisses_lua_fire_damage:OnRemoved()
    if IsServer() then 
        StopSoundOn("Hero_Snapfire.MortimerBlob.Target", self:GetParent())
    end
end

function modifier_snapfire_mortimer_kisses_lua_fire_damage:OnIntervalThink()
    local parent = self:GetParent()
    local caster = self:GetCaster()
    local ability = self:GetAbility()
...
function modifier_snapfire_mortimer_kisses_lua_fire_damage:DeclareFunctions()
    local funcs = { MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE}
    return funcs
end

function modifier_snapfire_mortimer_kisses_lua_fire_damage:GetModifierMoveSpeedBonus_Percentage()
    return self.slow_ms
end

function modifier_snapfire_mortimer_kisses_lua_fire_damage:GetEffectName()
    return "particles/units/heroes/hero_snapfire/hero_snapfire_burn_debuff.vpcf"
end

function modifier_snapfire_mortimer_kisses_lua_fire_damage:IsPurgable()
    return false
end

function modifier_snapfire_mortimer_kisses_lua_fire_damage:IsDebuff()
    return true
end

modifier_snapfire_mortimer_kisses_lua_talent = class({})

function modifier_snapfire_mortimer_kisses_lua_talent:OnCreated(table)
    self.damage_reduction = self:GetCaster():FindTalentValue("")
end

function modifier_snapfire_mortimer_kisses_lua_talent:OnRefresh(table)
    self.damage_reduction = self:GetCaster():FindTalentValue("")
end

function modifier_snapfire_mortimer_kisses_lua_talent:DeclareFunctions()
    local funcs = { MODIFIER_PROPERTY_INCOMING_DAMAGE_PERCENTAGE}
    return funcs
end

function modifier_snapfire_mortimer_kisses_lua_talent:GetModifierIncomingDamage_Percentage()
