kunkka_high_tide = class({})

function kunkka_high_tide:OnSpellStart()
...
function kunkka_high_tide:OnProjectileHit(hTarget, vLocation)
    if hTarget ~= nil and not hTarget:IsMagicImmune() and not hTarget:IsInvulnerable() then
        ...
        hTarget:AddNewModifier(caster, self, "modifier_kunkka_high_tide_debuff", {duration = debuff_dur})
...
modifier_kunkka_high_tide_debuff = class({})
LinkLuaModifier("modifier_kunkka_high_tide_debuff", "heroes/hero_kunkka/kunkka_high_tide", LUA_MODIFIER_MOTION_NONE)

function modifier_kunkka_high_tide_debuff:IsDebuff()
    return true
end

function modifier_kunkka_high_tide_debuff:IsPurgable()
    return true
end

function modifier_kunkka_high_tide_debuff:OnCreated()
    self:OnRefresh()
end

function modifier_kunkka_high_tide_debuff:OnRefresh()
...
function modifier_kunkka_high_tide_debuff:DeclareFunctions()
    return
    {
...
function modifier_kunkka_high_tide_debuff:GetModifierMagicalResistanceBonus()
    if self:GetSpecialValueFor("mariner") ~=0 then
        return -self.mres_reduc
    else return end
end

function modifier_kunkka_high_tide_debuff:GetModifierBaseDamageOutgoing_Percentage()
    if self:GetSpecialValueFor("admiral") ~= 0 then
        return -self.total_dmg_reduc
    else return end
end

function modifier_kunkka_high_tide_debuff:GetModifierPhysicalArmorBonus()
