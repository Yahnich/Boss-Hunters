...
function modifier_sylph_immaterialize_buff:OnCreated()
	...
	if IsServer() and self:GetParent():HasTalent("sylph_immaterialize_talent_1") then self:StartIntervalThink(0.1) end
end
...
function modifier_sylph_immaterialize_buff:CheckState()
...
function modifier_sylph_immaterialize_talent_slow:OnCreated()
	self.speed = self:GetCaster():FindTalentValue("sylph_immaterialize_talent_1")
end
