venomancer_venomous_ebf = class({})

function venomancer_venomous_ebf:GetIntrinsicModifierName()
	return "modifier_venomancer_venomous_handler"
end

LinkLuaModifier( "modifier_venomancer_venomous_handler", "heroes/hero_venomancer/venomancer_venomous_ebf", LUA_MODIFIER_MOTION_NONE )
...
function modifier_venomancer_venomous_handler:OnRefresh()
	...
	self.talent1 = self:GetParent():HasTalent("special_bonus_unique_venomancer_venomous_1")
	self.talent1Val = self:GetParent():FindTalentValue("special_bonus_unique_venomancer_venomous_1")
end
...
function modifier_venomancer_venomous_handler:OnAttackLanded(params)
	if IsServer() then
		if params.attacker == self:GetParent() then
			if params.target:HasModifier("modifier_venomancer_venomous_cancer") then
			...
			elseif params.target:IsAlive() then
				...
				local modifier = params.target:AddNewModifier(caster, caster:FindAbilityByName("venomancer_venomous_ebf"), "modifier_venomancer_venomous_cancer", {duration = self.duration})
		...
		if params.target == self:GetParent() and self.talent1 then
			...
			if params.attacker:HasModifier("modifier_venomancer_venomous_cancer") then
			...
			elseif params.attacker:IsAlive() then
				...
				local modifier = params.attacker:AddNewModifier(caster, self:GetAbility(), "modifier_venomancer_venomous_cancer", {duration = self.duration})
				modifier:SetStackCount(stacks)
			end
		end
	end
...
LinkLuaModifier( "modifier_venomancer_venomous_cancer", "heroes/hero_venomancer/venomancer_venomous_ebf", LUA_MODIFIER_MOTION_NONE)
...
function modifier_venomancer_venomous_cancer:OnRefresh()
	...
	self.talent2 = self:GetCaster():HasTalent("special_bonus_unique_venomancer_venomous_2")
	self.talent2Val = self:GetCaster():FindTalentValue("special_bonus_unique_venomancer_venomous_2")
end
	...
	if IsServer() then
		...
		if self.talent2 then
			for _, enemy in ipairs( caster:FindEnemyUnitsInRadius(parent:GetAbsOrigin(), self.talent2Val) ) do
				if enemy:HasModifier("modifier_venomancer_venomous_cancer") then
					local modifier = enemy:FindModifierByName("modifier_venomancer_venomous_cancer")
				...
				elseif enemy:IsAlive() then
					if not caster:IsHero() then caster = caster:GetOwnerEntity() end
					local modifier = enemy:AddNewModifier(caster, caster:FindAbilityByName("venomancer_venomous_ebf"), "modifier_venomancer_venomous_cancer", {duration = self.duration})
