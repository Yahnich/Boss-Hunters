...
function modifier_witch_doctor_voodoo_restoration_bh_handler:OnCreated()
	self.interval = self:GetSpecialValueFor("tick_interval")
	self.manaCost = self:GetSpecialValueFor("mana_per_second") * self.interval
...
function modifier_witch_doctor_voodoo_restoration_bh_heal:OnRefresh()
	...
	self.bonusHeal = self.heal * self:GetSpecialValueFor("bonus_damage") / 100
	if IsServer() then
...
function modifier_witch_doctor_voodoo_restoration_bh_curse:OnRefresh()
	self.interval = self:GetSpecialValueFor("tick_interval")
	self.damage = ( self:GetSpecialValueFor("damage") ) * self.interval
	self.bonusDamage = self.damage * self:GetSpecialValueFor("bonus_damage") / 100
