...
function broodmother_spawn_spiderlings_bh:OnSpellStart()
	...
	self:FireTrackingProjectile("particles/units/heroes/hero_broodmother/broodmother_web_cast.vpcf", target, self:GetSpecialValueFor("speed"), {}, DOTA_PROJECTILE_ATTACHMENT_ATTACK_1, false, true, 50)
end
...
function broodmother_spawn_spiderlings_bh:OnProjectileHit(hTarget, vLocation)
	...
	if hTarget and not hTarget:TriggerSpellAbsorb( self ) then
		...
		hTarget:AddNewModifier(caster, self, "modifier_broodmother_spawn_spiderlings_bh", {Duration = self:GetSpecialValueFor("duration")})
...
function modifier_broodmother_spawn_spiderlings_bh:OnRemoved()
    if IsServer() then
    	...
    	local damage = self:GetSpecialValueFor("spider_damage")
		local hp = self:GetSpecialValueFor("spider_health")
		local mr = self:GetSpecialValueFor("spider_mr")
    	...
    	for i=1,self:GetSpecialValueFor("count") do
    		...
    		local spider = caster:CreateSummon("npc_dota_broodmother_spiderling", position, self:GetSpecialValueFor("spider_duration"))
