...
function sven_valiant_charge:OnSpellStart()
	...
	caster:AddNewModifier(caster, self, "modifier_sven_valiant_charge", {duration = CalculateDistance(position, caster)/self:GetTalentSpecialValueFor("movespeed") })
...
function modifier_sven_valiant_charge:OnIntervalThink()
	local parent = self:GetParent()
	for _, enemy in ipairs( parent:FindEnemyUnitsInRadius( parent:GetAbsOrigin(), parent:GetHullRadius() + parent:GetCollisionPadding() + 32 ) ) do
