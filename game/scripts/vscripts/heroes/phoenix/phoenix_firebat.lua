phoenix_firebat = class({})

function phoenix_firebat:GetAbilityTextureName()
	if self:GetCaster():HasModifier("modifier_phoenix_kindled_soul_active") then
		return "custom/phoenix_firebat_kindled"
	else
		return "custom/phoenix_firebat"
	end
end

function phoenix_firebat:OnSpellStart()
	...
	caster:AddNewModifier(caster, self, "modifier_phoenix_firebat_invuln", {})
	EmitSoundOn("Hero_EmberSpirit.FlameGuard.Cast", caster)
	
	local internalTimer = 0.1 -- proc on first cast
	Timers:CreateTimer(0, function()
		if CalculateDistance(caster, position) > speed and distanceTraveled < distance then
			if caster:HasTalent("phoenix_firebat_talent_1") then direction = caster:GetForwardVector() end
			...
			if internalTimer > 0.1 then
				local duration = self:GetTalentSpecialValueFor("burn_duration")
				if caster:HasScepter() then duration = duration / 2 end
				for _, enemy in ipairs(caster:FindEnemyUnitsInRadius(caster:GetAbsOrigin(), radius, {})) do
					enemy:AddNewModifier(caster, self, "modifier_phoenix_firebat_fire_debuff", {duration = duration})
				end
			end
			return FrameTime()
		else
			...
			caster:RemoveModifierByName("modifier_phoenix_firebat_invuln")
			return nil
		end
	end)
end
...
modifier_phoenix_firebat_invuln = class({})
LinkLuaModifier("modifier_phoenix_firebat_invuln", "heroes/phoenix/phoenix_firebat.lua", 0)

function modifier_phoenix_firebat_invuln:OnCreated()
	if IsServer() then
		self:GetAbility():SetActivated(false)
	end
end

function modifier_phoenix_firebat_invuln:OnRefresh()
	if IsServer() then
		self:GetAbility():SetActivated(false)
	end
end

function modifier_phoenix_firebat_invuln:OnDestroy()
	if IsServer() then
		self:GetAbility():SetActivated(true)
	end
end

function modifier_phoenix_firebat_invuln:CheckState()
...
function modifier_phoenix_firebat_invuln:GetEffectName()
...
LinkLuaModifier( "modifier_phoenix_firebat_fire_debuff", "heroes/phoenix/phoenix_firebat.lua", LUA_MODIFIER_MOTION_NONE )
modifier_phoenix_firebat_fire_debuff = class({})

function modifier_phoenix_firebat_fire_debuff:OnCreated()
...
function modifier_phoenix_firebat_fire_debuff:OnRefresh()
	self.damage_over_time = self.damage_over_time + self:GetAbility():GetTalentSpecialValueFor("burn_dot")
	if self:GetCaster():HasScepter() then self.damage_over_time = self.damage_over_time * 2 end
end

function modifier_phoenix_firebat_fire_debuff:OnIntervalThink()
...
function modifier_phoenix_firebat_fire_debuff:IsDebuff()
	return true
end

--------------------------------------------------------------------------------

function modifier_phoenix_firebat_fire_debuff:GetEffectName()
	return "particles/units/heroes/hero_jakiro/jakiro_liquid_fire_debuff.vpcf"
end


function modifier_phoenix_firebat_fire_debuff:IsFireDebuff()
	return true
end