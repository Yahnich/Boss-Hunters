...
function phoenix_solar_jet:CreateSolarJet(position)
	...
	if caster:HasTalent("phoenix_solar_jet_talent_1") then
		for i = 1, 2 do
			local newXPos = position + vDirX * self.area_of_effect * 0.75 * (-1)^i
			local newYPos = position + vDirY * self.area_of_effect * 0.75 * (-1)^i
...
function modifier_phoenix_solar_jet_thinker:OnCreated( kv )
	...
	if self:GetCaster():HasScepter() then self.dot_duration = self.dot_duration / 2 end
	if IsServer() then
