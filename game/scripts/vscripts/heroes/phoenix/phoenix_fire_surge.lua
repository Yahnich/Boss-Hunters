phoenix_fire_surge = class({})

--------------------------------------------------------------------------------
function phoenix_fire_surge:GetAbilityTextureName()
	if self:GetCaster():HasModifier("modifier_phoenix_kindled_soul_active") then
		return "custom/phoenix_fire_surge_kindled"
	else
		return "custom/phoenix_fire_surge"
	end
end

function phoenix_fire_surge:OnSpellStart()
...
function phoenix_fire_surge:CreateFireSurge(vDirection)
	local info = {
...
function phoenix_fire_surge:OnProjectileHitHandle( hTarget, vLocation, projectileID )
	if hTarget ~= nil and ( not hTarget:IsMagicImmune() ) and ( not hTarget:IsInvulnerable() ) then
		...
		ApplyDamage( damage )
		hTarget:AddNewModifier( self:GetCaster(), self, "modifier_phoenix_fire_surge_fire_debuff", { duration = self:GetTalentSpecialValueFor("dot_duration"), damage = self.projectileTable[projectileID] * self:GetTalentSpecialValueFor("dot_dmg_pct") / 100} )
...
function phoenix_fire_surge:OnProjectileThinkHandle( projectileID )
	if not self:GetCaster():HasTalent("phoenix_fire_surge_talent_1") then
		self.projectileTable[projectileID] = self.projectileTable[projectileID] or self.maxDamage
...
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
LinkLuaModifier( "modifier_phoenix_fire_surge_fire_debuff", "heroes/phoenix/phoenix_fire_surge.lua", LUA_MODIFIER_MOTION_NONE )
modifier_phoenix_fire_surge_fire_debuff = class({})

function modifier_phoenix_fire_surge_fire_debuff:OnCreated(kv)
...
function modifier_phoenix_fire_surge_fire_debuff:OnRefresh(kv)
	self.damage_over_time = tonumber(kv.damage)
	if self:GetCaster():HasScepter() then self.damage_over_time = self.damage_over_time * 2 end
end

function modifier_phoenix_fire_surge_fire_debuff:OnIntervalThink()
...
function modifier_phoenix_fire_surge_fire_debuff:IsDebuff()
	return true
end

--------------------------------------------------------------------------------

function modifier_phoenix_fire_surge_fire_debuff:GetEffectName()
	return "particles/units/heroes/hero_jakiro/jakiro_liquid_fire_debuff.vpcf"
end


function modifier_phoenix_fire_surge_fire_debuff:IsFireDebuff()
	return true
end