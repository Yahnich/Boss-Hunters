...
function phoenix_fire_surge:OnProjectileHitHandle( hTarget, vLocation, projectileID )
	if hTarget ~= nil and ( not hTarget:IsMagicImmune() ) and ( not hTarget:IsInvulnerable() ) then
		...
		hTarget:AddNewModifier( self:GetCaster(), self, "modifier_phoenix_fire_surge_fire_debuff", { duration = duration, damage = self.projectileTable[projectileID] * self:GetTalentSpecialValueFor("dot_dmg_pct") / 100} )
