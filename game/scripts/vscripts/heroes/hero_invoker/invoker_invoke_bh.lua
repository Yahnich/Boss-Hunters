invoker_invoke_bh = class({})
LinkLuaModifier("modifier_invoker_invoke_bh", "heroes/hero_invoker/invoker_invoke_bh", LUA_MODIFIER_MOTION_NONE)

function invoker_invoke_bh:IsStealable()
    return false
end

function invoker_invoke_bh:IsHiddenWhenStolen()
    return false
end

function invoker_invoke_bh:OnSpellStart()
    local caster = self:GetCaster()

    -- Ability variables
    caster.invoked_orbs = caster.invoked_orbs or {}
    local max_invoked_spells = self:GetSpecialValueFor("max_invoked_spells")
    ...
    if caster.invoked_orbs[1] and caster.invoked_orbs[2] and caster.invoked_orbs[3] then
        ...
        for i=1, 3, 1 do
            if caster.invoked_orbs[i]:GetName() == "invoker_q" then
                num_quas_orbs = num_quas_orbs + 1
            elseif caster.invoked_orbs[i]:GetName() == "invoker_w" then
                num_wex_orbs = num_wex_orbs + 1
            elseif caster.invoked_orbs[i]:GetName() == "invoker_e" then
