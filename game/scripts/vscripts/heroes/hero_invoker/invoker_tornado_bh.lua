...
function invoker_tornado_bh:OnSpellStart()
    ...
    local travel_distance = self:GetTalentSpecialValueFor("travel_distance")
...
function invoker_tornado_bh:OnProjectileHit(hTarget, vLocation)
    ...
    if hTarget then
        ...
        local wex_damage = self:GetTalentSpecialValueFor("wex_damage")
        ...
        AddFOWViewer(caster:GetTeam(), vLocation, 200, self:GetTalentSpecialValueFor("end_vision_duration"), true)
    end
end