ifrit_searing_heat = class({})

function ifrit_searing_heat:GetAbilityTextureName()
	if self:GetCaster():HasModifier("modifier_ifrit_kindled_soul_active") then
		return "custom/ifrit_searing_heat_kindled"
	else
		return "custom/ifrit_searing_heat"
	end
end

function ifrit_searing_heat:OnSpellStart()
...
function ifrit_searing_heat:CreateSearingHeat(target, origin)
	local caster = self:GetCaster()
	...
	if caster:HasTalent("ifrit_searing_heat_talent_1") then
		...
		if newTarget then
			Timers:CreateTimer(self.delay, function()
				...
				Timers:CreateTimer(0.25, function() if not newTarget:IsAlive() then self:Refresh() end end)
			end)
		end
	end
end