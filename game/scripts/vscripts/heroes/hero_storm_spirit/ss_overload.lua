...
function modifier_ss_overload_toggle:OnRefresh(table)
	...
	self.damage_per_lvl = self:GetSpecialValueFor("damage_per_lvl")
	self.radius = self:GetSpecialValueFor("radius")
	self.tick = self:GetSpecialValueFor("scepter_tick")
	self.mana = self:GetSpecialValueFor("scepter_mana_gain")
	if IsServer() then
...
function modifier_ss_overload_attack:OnCreated(table)
	if IsServer() then
		...
		local damage_per_lvl = self:GetSpecialValueFor("damage_per_lvl") * self:GetCaster():GetLevel()
...
function modifier_ss_overload_attack:OnAttackLanded(params)
	if IsServer() then
		...
		if attacker == parent then
			...
			local enemies = parent:FindEnemyUnitsInRadius(target:GetAbsOrigin(), self:GetSpecialValueFor("radius"))
			for _,enemy in pairs(enemies) do
