...
function bh_shuriken:OnSpellStart()
	...
	self:TossShuriken(target, self:GetSpecialValueFor("damage"), caster, self.shadow_walk)
	...
	if caster:HasTalent("special_bonus_unique_bh_shuriken_2") then
		...
		for _,enemy in pairs(enemies) do
			if enemy ~= target then
				self:TossShuriken(enemy, self:GetSpecialValueFor("damage"), caster, self.shadow_walk)
...
function bh_shuriken:TossShuriken(target, damage, source, bShadowWalk, hExtraData)
	...
	self:FireTrackingProjectile( "particles/units/heroes/hero_bounty_hunter/bounty_hunter_suriken_toss.vpcf", target, self:GetSpecialValueFor("speed"), {extraData = extraData, source = hSource, origin = hSource:GetAbsOrigin() }, DOTA_PROJECTILE_ATTACHMENT_ATTACK_1, true, false, 0)
end
...
function bh_shuriken:OnProjectileHit_ExtraData( target, position, extraData )
	if target then
		...
		local maxBounces = self:GetSpecialValueFor("max_bounces")
		...
		if caster:HasScepter() then
			ministun = self:GetSpecialValueFor("scepter_ministun")
		end
		
		if shadow_walk then
			local shadowWalk = caster:FindAbilityByName("bh_shadow_walk")
			damage = damage + shadowWalk:GetSpecialValueFor("damage")
			target:AddNewModifier( caster, shadowWalk, "modifier_bh_shadow_walk_slow", {duration = shadowWalk:GetSpecialValueFor("slow_duration")} )
		end
