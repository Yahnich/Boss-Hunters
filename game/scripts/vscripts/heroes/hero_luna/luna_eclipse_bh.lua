...
function modifier_luna_eclipse_bh:OnCreated()
	...
	self.hits = self:GetTalentSpecialValueFor("hit_count")
	...
	if self.lucent then
		for _, enemy in ipairs( caster:FindEnemyUnitsInRadius( position, self.radius) ) do
			...
			if self.hitEnemies[enemy:entindex()] < self.hits then
				self.lucent:LucentBeam(enemy, self.beam_radius, self.stun_dur )
		...
		self.lucent:LucentBeamPosition( randomPosition, self.beam_radius, self.stun_dur )
	end
