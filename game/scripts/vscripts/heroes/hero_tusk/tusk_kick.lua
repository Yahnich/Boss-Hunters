...
function modifier_tusk_kick:OnIntervalThink()
    GridNav:DestroyTreesAroundPoint(self:GetParent():GetAbsOrigin(), self:GetParent():GetHullRadius()+self:GetTalentSpecialValueFor("width"), false)
    local enemies = self:GetCaster():FindEnemyUnitsInRadius(self:GetParent():GetAbsOrigin(), self:GetParent():GetHullRadius()+self:GetTalentSpecialValueFor("width"))
    for _,enemy in pairs(enemies) do
