...
function puppeteer_mass_of_flesh:OnSpellStart()
	...
	local collectionDummy = caster:CreateSummon("npc_dota_puppeteer_mass_of_flesh", position, self:GetSpecialValueFor("duration"))
...
function modifier_wall_of_flesh_segment_aura:OnCreated()
	...
	self.slow = self:GetAbility():GetSpecialValueFor("aura_slow")
end
...
function modifier_wall_of_flesh_segment_aura:GetModifierMoveSpeedBonus_Percentage()
	return self.slow
end