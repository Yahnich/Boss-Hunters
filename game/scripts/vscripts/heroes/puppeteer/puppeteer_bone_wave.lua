...
function puppeteer_bone_wave:OnSpellStart()
	...
	caster:AddNewModifier(caster, self, "modifier_puppeteer_bone_wave", {duration = self:GetSpecialValueFor("duration")})
...
function modifier_puppeteer_bone_wave:OnCreated()
	self.speed = self:GetAbility():GetSpecialValueFor("bonus_ms")
	self.damage = self:GetAbility():GetSpecialValueFor("effect_damage")
	self.radius = self:GetAbility():GetSpecialValueFor("effect_radius")
	self.knockback = self:GetAbility():GetSpecialValueFor("knockback_max")
	if IsServer() then
