...
function dazzle_poison_touch_bh:OnSpellStart()
	...
	local length = self:GetSpecialValueFor("end_distance") + caster:GetHullRadius() + caster:GetCollisionPadding()
...
function dazzle_poison_touch_bh:OnProjectileHit( target, position )
	if target and not target:TriggerSpellAbsorb(self) then
		local caster = self:GetCaster()
		target:AddNewModifier( caster, self, "modifier_dazzle_poison_touch_bh", {duration = self:GetSpecialValueFor("duration")} )
	end
end
...
function modifier_dazzle_poison_touch_bh:OnCreated()
	...
	self.tick = self:GetDuration() / self:GetSpecialValueFor("duration")
...
function modifier_dazzle_poison_touch_bh:OnRefresh()
	...
	self.radius = self:GetCaster():FindTalentValue("special_bonus_unique_dazzle_poison_touch_1", "radius")
	if IsServer() then
		self:StartIntervalThink( self:GetDuration() / self:GetSpecialValueFor("duration") )
	end
end
