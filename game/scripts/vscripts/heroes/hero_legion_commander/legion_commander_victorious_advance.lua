...
function legion_commander_victorious_advance:OnSpellStart()
	if IsServer() then
		...
		local bonus_damage_minion = self:GetSpecialValueFor("damage_per_minion")
		local base_movespeed = 0
		local movespeed = self:GetSpecialValueFor("bonus_speed")
		local movespeed_minion = self:GetSpecialValueFor("bonus_speed_minion")
