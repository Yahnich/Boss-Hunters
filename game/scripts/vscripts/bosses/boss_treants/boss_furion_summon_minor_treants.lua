...
function boss_furion_summon_minor_treants:OnSpellStart()
...
function boss_furion_summon_minor_treants:OnChannelFinish(bInterrupted)
	local caster = self:GetCaster()
	...
	if not bInterrupted then
		...
		Timers:CreateTimer(0.1, function()
			...
			local treant = CreateUnitByName("npc_dota_minion_minor_treant", treantPos, true, caster, caster, caster:GetTeamNumber())
...
function boss_furion_summon_minor_treants:GetTreantCount()
	local treants = 0
	self.treants = self.treants or {}
	for treant,_ in pairs(self.treants) do
		if treant:IsNull() or not treant:IsAlive() then
...
function boss_furion_summon_minor_treants:GetTreants()
