...
function modifier_boss_necro_swans_song:OnIntervalThink()
	...
	for _, hero in ipairs( heroes ) do
		self:GetAbility():DealDamage( self:GetCaster(), hero, hero:GetMaxHealth() * self.damage * HeroList:GetActiveHeroCount() / #heroes, {damage_type = DAMAGE_TYPE_PURE, damage_flags = DOTA_DAMAGE_FLAG_HPLOSS + DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION} )
	end
end
