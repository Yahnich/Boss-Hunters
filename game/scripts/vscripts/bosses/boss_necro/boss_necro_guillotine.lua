		...
		for _, enemy in ipairs( caster:FindEnemyUnitsInRadius( position, radius, {type = DOTA_UNIT_TARGET_HERO} ) ) do
			if not enemy:TriggerSpellAbsorb(self) then
				if enemy:GetHealthPercent() <= kill_threshold then
					...
					self:DealDamage( caster, enemy, enemy:GetHealth() * damage, {damage_type = DAMAGE_TYPE_PURE, damage_flags = DOTA_DAMAGE_FLAG_BYPASSES_ALL_BLOCK  + DOTA_DAMAGE_FLAG_NON_LETHAL + DOTA_DAMAGE_FLAG_NO_DAMAGE_MULTIPLIERS + DOTA_DAMAGE_FLAG_HPLOSS + DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION} )
