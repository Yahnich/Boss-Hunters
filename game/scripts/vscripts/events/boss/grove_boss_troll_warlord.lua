local function OnEntityKilled(self, event)
	...
	if killedTarget:GetUnitName() == "npc_dota_boss_troll_warlord" then
...
local function StartEvent(self)
	...
	self.minionHandler = Timers:CreateTimer(5, function()
		if not self.eventEnded then
			if self.minionsAlive < ( math.floor( math.log( RoundManager:GetRaidsFinished() + 1) + 1 ) ) * GameRules.gameDifficulty and self.bossesAlive > 0 then
