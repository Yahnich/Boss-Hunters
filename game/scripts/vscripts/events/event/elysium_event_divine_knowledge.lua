...
local function FirstChoice(self, userid, event)
	...
	hero:AddNewModifier(hero, nil, "event_buff_divine_knowledge_1", {})
...
local function SecondChoice(self, userid, event)
	local hero = PlayerResource:GetSelectedHeroEntity( event.pID )
	hero:AddNewModifier(hero, nil, "event_buff_divine_knowledge_2", {})
	self._playerChoices[event.pID] = true
	CheckPlayerChoices(self)
end
...
local function ThirdChoice(self, userid, event)
	local hero = PlayerResource:GetSelectedHeroEntity( event.pID )
	hero:AddNewModifier(hero, nil, "event_buff_divine_knowledge_3", {})
	self._playerChoices[event.pID] = true
	CheckPlayerChoices(self)
end
